define([
    ...
    function main(config, element) {
        ...
        $('#vat_tax_id, #customer_code').on('change keyup', function () {
            ...
            if (tax_value.length >= minlength && tax_value != ''  && customer_code != '' ) {
                $.ajax({
                    url: AjaxUrl,
                    type: "GET",
                    ...
                    if (data.success == 'false') { 
                        ...
                        $("#company_name").val('');
                        $('#link-error').html("Cusomer is not linked. Please enter valid customer code");  // set the message as html of span
                    } 
                    return true;
                });}
            });
