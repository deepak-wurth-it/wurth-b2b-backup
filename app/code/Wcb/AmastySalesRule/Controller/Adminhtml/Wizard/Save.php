...
namespace Wcb\AmastySalesRule\Controller\Adminhtml\Wizard;
use Amasty\SalesRuleWizard\Controller\Adminhtml\Wizard\Save as AmastySalesRules;
class Save extends AmastySalesRules
{
    /**
    ...
    public function __construct(
        ...
        parent::__construct($context,$ruleFactory,$session);
    }
    ...
    public function execute()
    {
        ...
        $ruleData['extension_attributes']['ampromo_rule']['discount_products_quantity'] = $this->combineDiscountWithSku($data);
     ...
     * @return string
     */
    protected function combineDiscountWithSku($data)
    {
        if (!isset($data['rule_settings']['free_gifts']['products'])) {
            return '';
        }
        ...
        $discount_quantity = [];
        foreach ($grid as $product) {
            if($product['discount_quantity']){
               $discount_quantity[$product['sku']] = $product['discount_quantity'];
            }
            
          }

        return json_encode($discount_quantity);
    }
