<?php
/**
 * Copyright ©  All rights reserved.
 * See COPYING.txt for license details.
 */
declare(strict_types=1);

namespace Wurth\Landingpage\Controller\Adminhtml\Landingpage;
use Wurth\Landingpage\Model\LandingPageFactory;

class Edit extends \Wurth\Landingpage\Controller\Adminhtml\Landingpage
{
    protected $landingPageFactory;

    protected $resultPageFactory;

    /**
     * @param \Magento\Backend\App\Action\Context $context
     * @param \Magento\Framework\Registry $coreRegistry
     * @param \Magento\Framework\View\Result\PageFactory $resultPageFactory
     */
    public function __construct(
        \Magento\Backend\App\Action\Context $context,
        \Magento\Framework\Registry $coreRegistry,
        \Magento\Framework\View\Result\PageFactory $resultPageFactory,
        LandingPageFactory $landingPageFactory
    ) {
        $this->landingPageFactory = $landingPageFactory;
        $this->resultPageFactory = $resultPageFactory;
        parent::__construct($context, $coreRegistry);
    }

    /**
     * Edit action
     *
     * @return \Magento\Framework\Controller\ResultInterface
     */
    public function execute()
    {
        // 1. Get ID and create model
        $id = $this->getRequest()->getParam('landing_page_id');
        $model = $this->landingPageFactory->create();

        // 2. Initial checking
        if ($id) {
            $model->load($id);
            if (!$model->getId()) {
                $this->messageManager->addErrorMessage(__('This Landing Page no longer exists.'));
                /** @var \Magento\Backend\Model\View\Result\Redirect $resultRedirect */
                $resultRedirect = $this->resultRedirectFactory->create();
                return $resultRedirect->setPath('*/*/');
            }
        }
        $this->_coreRegistry->register('wurth_landingpage_landing_page', $model);

        // 3. Build edit form
        /** @var \Magento\Backend\Model\View\Result\Page $resultPage */
        $resultPage = $this->resultPageFactory->create();
        $this->initPage($resultPage)->addBreadcrumb(
            $id ? __('Edit Landing Page') : __('New Landing Page'),
            $id ? __('Edit Landing Page') : __('New Landing Page')
        );
        $resultPage->getConfig()->getTitle()->prepend(__('Landing Pages'));
        $resultPage->getConfig()->getTitle()->prepend($model->getId() ? __('Edit Landing Page %1', $model->getTitle()) : __('New Landing Page'));
        return $resultPage;
    }
}
