...
class AdminNotificationFeed extends \Magento\AdminNotification\Model\Feed
{
    ...
    public function getFeedUrl()
    {
        ...
        $modulesParams = array();
        foreach($this->getPlazathemesModules() as $key => $module) {
            $key = str_replace('Plazathemes_', '', $key);
    ...
    /**
     * Get Plazathemes Modules Info
     *
     * @return $this
     */
    protected function getPlazathemesModules()
    {
        $modules = array();
        foreach($this->_moduleList->getAll() as $moduleName => $module) {
            if ( strpos($moduleName, 'Plazathemes_') !== false && $this->_moduleManager->isEnabled($moduleName) ) {
    ...
    {
        return $this->_cacheManager->load('Plazathemes_admin_notifications_lastcheck');
    }
    ...
    {
        $this->_cacheManager->save(time(), 'plazathemes_admin_notifications_lastcheck');
        return $this;
    }
